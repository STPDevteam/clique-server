...
func GetV1Proposal() {
	defer time.AfterFunc(time.Duration(60)*time.Second, GetV1Proposal)
	...
	var v1Url = fmt.Sprintf(viper.GetString("app.api_v1_proposal_url"), v1StartId)
	...
	if res.Data != nil {
		for _, data := range res.Data {
			...
			if countExist == 0 {
				...
				resContent, err := utils.QueryMethodEthCallByTag(data.Address, dataParam, viper.GetString("app.api_v1_proposal_content_url"), "latest")
				if err != nil {
...
func saveV1Proposal(decode []interface{}, data models.V1ProposalData) error {
	...
	var entities []db.TbProposalV1Model
