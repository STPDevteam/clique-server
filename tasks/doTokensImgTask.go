...
func SwapTokenPrice() {
	defer time.AfterFunc(time.Duration(600)*time.Second, SwapTokenPrice)

	var swapTokenArr []db.TbSwapToken
	...
	res, ok := config.MyCache.Get("swap_token_price")
	if !ok {
		resId, err = utils.GetTokensId("https://api.coingecko.com/api/v3/coins/list?include_platform=true")
		if err != nil {
			oo.LogW("GetTokensId failed error: %v", err)
		...
		config.MyCache.Set("swap_token_price", resId, time.Duration(60)*time.Minute)
	} else {
